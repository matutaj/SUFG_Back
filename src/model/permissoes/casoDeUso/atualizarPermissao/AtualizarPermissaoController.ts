...
class AtualizarPermissaoController {
  async handle(req: Request, res: Response): Promise<any> {
    ...
    if (!(await atualizarPermissaoSchema.isValid(req.params)))
      throw new AppError("Erro na Validação dos dados");
    if (!(await atualizarPermissaoSchema.isValid(req.body)))
      throw new AppError("Erro na Validação dos dados");
    const result = await atualizarPermissaoCasoDeUso.execute({
      id,
      descricao,
      nome,
    });
