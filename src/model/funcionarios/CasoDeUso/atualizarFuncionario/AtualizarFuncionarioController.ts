...
class AtualizarFuncionarioController {
    async handle(req: Request, res: Response): Promise<any> {
        ...
        if (!atualizarFuncionarioSchema.isValid(req.body)) throw new AppError("Erro na Validação dos dados");
        if (!atualizarFuncionarioSchema.isValid(req.params)) throw new AppError("Erro na Validação dos dados");
