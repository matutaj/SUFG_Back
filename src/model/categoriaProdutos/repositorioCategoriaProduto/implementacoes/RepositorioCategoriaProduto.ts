...
class CategoriaProdutoRepositorio implements ICategoriaProduto {
  ...
  async listarTodasCategoriasProdutos(): Promise<categoriasProdutos[]> {
    const listarTodasCategoriasProdutos =
      await prisma.categoriasProdutos.findMany();
    return listarTodasCategoriasProdutos;
  }
  async listarUmaCategoriaProdutoPeloId(
    id: string
  ): Promise<categoriasProdutos | undefined> {
  ...
  async listarUmaCategoriaProdutoPeloNome(
    nomeCategoria: string
  ): Promise<categoriasProdutos | undefined> {
    const listarUmaCategoriaProdutoPeloNome =
      (await prisma.categoriasProdutos.findFirst({
        where: { nomeCategoria },
      })) || undefined;
    return listarUmaCategoriaProdutoPeloNome;
  }
  async atualizarCategoriaProduto({
    descricao,
    nomeCategoria,
    id,
  }: DadosCategoriaProduto): Promise<categoriasProdutos> {
