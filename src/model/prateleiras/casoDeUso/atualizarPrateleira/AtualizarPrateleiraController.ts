...
class AtualizarPrateleiraController {
    async handle(req: Request, res: Response): Promise<any> {
        ...
        if (!atualizarPrateleiraSchema.isValid(req.params)) throw new AppError("Erro na Validação dos dados");
        if (!atualizarPrateleiraSchema.isValid(req.body)) throw new AppError("Erro na Validação dos dados");
