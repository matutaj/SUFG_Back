...
class FuncaoRepositorio implements IFuncao {
  ...
  async listarFuncaoPeloId(id: string): Promise<funcoes | undefined> {
    const listarFuncaoPeloId =
      (await prisma.funcoes.findUnique({ where: { id } })) || undefined;
  ...
  async listarFuncaoPeloNome(nome: string): Promise<funcoes | undefined> {
    const listarFuncaoPeloNome =
      (await prisma.funcoes.findFirst({ where: { nome: nome } })) || undefined;
