...
class ListarUmProdutoLocalizacaoPorIdController {
  async handle(req: Request, res: Response): Promise<any> {
    const listarUmProdutoLocalizacaoPorIdCasoDeUso =
      new ListarUmProdutoLocalizacaoPorIdCasoDeUso();
    const { id } = req.params;
    if (!(await listarProdutoLocalizacaoPeloIdSchema.isValid(req.params)))
