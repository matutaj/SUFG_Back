...
class FornecedorRepositorio implements IFornecedor {
  async criarFornecedor({
    ...
    const criarFornecedor = await prisma.fornecedores.create({
      data: {
        emailFornecedor,
        moradaFornecedor,
        nif,
  ...
  async listarUmFornecedorPeloId(
    id: string
  ): Promise<fornecedores | undefined> {
    const listarUmFornecedorPeloId =
      (await prisma.fornecedores.findUnique({ where: { id } })) || undefined;
    return listarUmFornecedorPeloId;
  }
  async listarUmFornecedorPeloNome(
    nomeFornecedor: string
  ): Promise<fornecedores | undefined> {
    const listarUmFornecedorPeloNome =
      (await prisma.fornecedores.findFirst({ where: { nomeFornecedor } })) ||
      undefined;
  ...
  async atualizarFornecedor({
    ...
    const atualizarFornecedor = await prisma.fornecedores.update({
      where: { id },
      data: {
        emailFornecedor,
        moradaFornecedor,
  ...
  async listarTelefoneFornecedor(
    telefoneFornecedor: number
  ): Promise<fornecedores | undefined> {
    const listarTelefoneFornecedor =
      (await prisma.fornecedores.findFirst({
        where: { telefoneFornecedor },
      })) || undefined;
  ...
  async listarNumeroDeContribuinte(
    ...
    const listarNumeroDeContribuinte =
      (await prisma.fornecedores.findFirst({ where: { nif } })) || undefined;
